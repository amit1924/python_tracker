...
@app.route('/edit/<string:client_id>', methods=['GET', 'POST'])
def edit_client(client_id):
    ...
    if request.method == 'POST':
        ...
        result = collection.update_one({'_id': ObjectId(client_id)}, {'$set': updated_data})
        
        if result.modified_count == 1:
            return redirect(url_for('index'))
        else:
            return render_template('edit.html', client=client_data, error_message="Update failed")
    
    return render_template('edit.html', client=client_data)

@app.route('/delete/<string:client_id>', methods=['POST'])
